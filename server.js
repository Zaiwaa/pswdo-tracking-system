...
async function notifyUsers(title,message,excludeId,type='info'){
  const users=await query("SELECT id FROM users WHERE status='Active' AND ($1::uuid IS NULL OR id<>$1::uuid)",[excludeId||null]);
  for(const u of users.rows) await query('INSERT INTO notifications(id,user_id,title,message,type) VALUES($1,$2,$3,$4,$5)',[uuidv4(),u.id,title,message,type]);
  io.emit('notification:new');
}
async function notifyUser(userId,title,message,type='info'){
  if(!userId)return; await query('INSERT INTO notifications(id,user_id,title,message,type) VALUES($1,$2,$3,$4,$5)',[uuidv4(),userId,title,message,type]); io.to(`user:${userId}`).emit('notification:new');
}

async function notifyAdmins(title,message,excludeId,type='approval'){
  ...
  for(const u of r.rows){
    await query('INSERT INTO notifications(id,user_id,title,message,type) VALUES($1,$2,$3,$4,$5)',[uuidv4(),u.id,title,message,type]);
...
app.post('/api/rpc/authRegister', async (req,res,next)=>{try{
  ...
  await notifyUsers('New account awaiting approval',`${fullName} (${position}) created an account.`,id,'account');
...
app.post('/api/track/:token/action', authenticate, async (req,res,next)=>{try{
  ...
  await notifyUsers('Document tracking update',`${req.user.full_name} ${checkpoint.toLowerCase()} ${d.reference_number}${office?` at ${office}`:''}.`,req.user.id,'document');
...
app.post('/api/rpc/:fn', authenticate, async (req,res,next)=>{try{
  ...
  switch(fn){
    ...
    case 'collabSendMessage': {
      const message=String(a[a.length-1]||'').trim(); if(!message)throw new Error('Message cannot be empty.'); if(message.length>1000)throw new Error('Message is too long.');
      await query('INSERT INTO messages(id,user_id,message) VALUES($1,$2,$3)',[uuidv4(),me.id,message]); await notifyUsers('New chat message',`${me.full_name} sent a message.`,me.id,'chat'); io.emit('chat:new'); result={success:true}; break;
    }
    case 'collabGetMessages': {
      const limit=Math.min(Number(a[a.length-1])||50,100); const r=await query(`SELECT m.id,m.user_id,u.full_name,u.position,m.message,m.created_at FROM messages m JOIN users u ON u.id=m.user_id ORDER BY m.created_at DESC LIMIT $1`,[limit]);
      result=r.rows.reverse().map(x=>({id:String(x.id),userId:String(x.user_id),fullName:x.full_name,position:x.position,message:x.message,createdAt:fmtDateTime(x.created_at)})); break;
    }
    case 'collabGetNotifications': {
      const r=await query('SELECT * FROM notifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT 50',[me.id]); result=r.rows.map(x=>({id:String(x.id),title:x.title,message:x.message,type:x.type,createdAt:fmtDateTime(x.created_at),read:!!x.read_at})); break;
    }
...
const codes={'Project Proposal':'PP','Financial Assistance':'FA','Payroll and Personnel':'PAP'};
async function generateReference(programUnit,documentType,dateCreated){
  const code=codes[documentType]; if(!code)throw new Error('Invalid document type.'); const d=String(dateCreated||'').replace(/-/g,'').slice(2); const programCode=String(programUnit||'OTHERS').replace(/[^A-Za-z0-9]/g,'').toUpperCase().slice(0,12)||'OTHERS';
  const r=await query('SELECT reference_number FROM documents WHERE program_unit=$1 AND document_type=$2',[programUnit,documentType]); let max=0; for(const x of r.rows){const m=x.reference_number.match(/-(\d{5})$/);if(m)max=Math.max(max,Number(m[1]));} return `${programCode}-${code}-${d}-${String(max+1).padStart(5,'0')}`;
}
...
async function saveDocument(data,me){ validateDocument(data); const id=uuidv4(), ref=data.referenceNumber||await generateReference(data.programUnit,data.documentType,data.dateCreated), routing=buildRouting(data.routingDetails||[],data.assignedLiaison||'',data.remarks||''), image=saveRemarkData(data); const remarks=[]; if(String(data.remarks||'').trim()||image)remarks.push({timestamp:fmtDateTime(),remark:String(data.remarks||'').trim(),imageUrl:image,imageName:data.remarkImageName||''});
  ...
  for(const ev of routing)await insertHistory(id,ref,ev); await audit(me.id,id,'DOCUMENT_CREATED',{referenceNumber:ref}); await notifyUsers('New document added',`${me.full_name} added ${ref}.`,me.id,'document'); return{success:true}; }
...
async function requestDeletion(targetType,documentId,targetKey,reason,me){
  ...
  await notifyAdmins('Deletion approval requested',`${me.full_name} requested removal of a ${targetType==='history'?'history entry':'remark'} from ${d.reference_number}.`,null,'approval');
...
async function adminReviewDeletionRequest(requestId,decision,note,me){
  ...
  await notifyUser(reqRow.requester_id,`Deletion request ${decision.toLowerCase()}`,`Your request to remove a ${reqRow.target_type==='history'?'history entry':'remark'} from ${ref} was ${decision.toLowerCase()}${note?`: ${note}`:'.'}`,'approval');
  if(decision==='Approved'){
