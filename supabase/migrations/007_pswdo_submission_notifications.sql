-- V5.2: guarantee that approved PSWDO users receive an in-app notification
-- whenever an LGU submits, resubmits, or submits a Zero Report.
-- Run after migrations 005 and 006.
...
begin
  ...
  -- Notify every approved PSWDO account. Each PSWDO gets its own unread record.
  insert into public.notifications(user_id,title,message,action_url)
  select p.id,
         case when r.status='RETURNED_FOR_CORRECTION'::public.report_status
              then 'LGU report resubmitted — '||coalesce(lgu_name,'LGU')
              else 'LGU report submitted — '||coalesce(lgu_name,'LGU') end,
         coalesce(lgu_name,'An LGU')||' submitted DROMIC Report Version '||vno||'. Open the report for PSWDO review.',
         '/review/'||p_report_id
  from public.profiles p
  where p.role='pswdo'::public.user_role
    and p.approved=true;
...
begin
  ...
  insert into public.notifications(user_id,title,message,action_url)
  select p.id,
         'LGU Zero Report submitted — '||coalesce(lgu_name,'LGU'),
         coalesce(lgu_name,'An LGU')||' submitted a ZERO / NO AFFECTED POPULATION report. Open it for PSWDO review.',
         '/review/'||p_report_id
  from public.profiles p
  where p.role='pswdo'::public.user_role
    and p.approved=true;
