...
async function initDb() {
  ...
  await query('ALTER TABLE messages ADD COLUMN IF NOT EXISTS recipient_id UUID REFERENCES users(id) ON DELETE CASCADE');
  await query('ALTER TABLE notifications ADD COLUMN IF NOT EXISTS action_url TEXT');
  await query('CREATE INDEX IF NOT EXISTS idx_messages_conversation ON messages(user_id, recipient_id, created_at DESC)');
